(function () {
  ...
  let hasElem = (school) => {
    if (typeof school.GRADE_ORG === 'number') {  // if number
      school.HAS_ELEMENTARY = school.GRADE_LEVEL <= 1;
    } else {  // otherwise (in case of string)
  ...
  let hasMid = (school) => {
    if (typeof school.GRADE_ORG === 'number') {  // if number
      ...
      school.HAS_KINDERGARTEN = school.GRADE_LEVEL.toUpperCase().indexOf('MID') >= 0;
    }  
    return school;  
  }
  ...
  let hasHigh = (school) => {
    if (typeof school.GRADE_ORG === 'number') {  // if number
      school.HAS_HIGH_SCHOOL = school.GRADE_LEVEL < 1;
    } else {  // otherwise (in case of string)
      school.HAS_HIGH_SCHOOL = school.GRADE_LEVEL.toUpperCase().indexOf('MID') >= 0;
    }  
    return school;  
  }
...
var isSchool = function(schoolData){
  return(isKindergarten(schoolData)||hasElem(schoolData)||hasMid(schoolData)||hasHigh(schoolData));
}
...
var meets_filtered_Conditions = function(schoolData){
  ...
  var filtered_out = [];	  var filtered_out = [];
  for (var i = 0; i < schools.length - 1; i++) {	  _.each(schools,function(schoolData){
    isOpen = schools[i].ACTIVE.toUpperCase() == 'OPEN';	    if (meets_filtered_Conditions(schoolData)) {
    isPublic = (schools[i].TYPE.toUpperCase() !== 'CHARTER' ||	 filtered_data.push(schoolData);
  ...
  //   The style options
  //   var pathOpts = {
  //     'radius': filtered_data[i].ENROLLMENT / 30,
  //     'fillColor': color
  //   };
...
_.each(filtered_data,function(schoolData){
  meets_filtered_Conditions(schoolData)
})

if (hasHigh(schoolData)){ 
  color = "red"
} else if (hasMid(schoolData)){
  color = "blue"
} else if (hasElem(schoolData)){ 
  color = "yellow"
}
  else{ 
    color = "orange"
